...
func setup_fitness(fname string) {
	...
	fmt.Println("varnumber = ", varnumber)
	var token2 []string
	for i := 0; i < fitnesscases; i++ {
		str, _ = reader.ReadString('\n')
		token2 = strings.Split(str, " ")
		targets[i] = make([]float64, varnumber+1)
		for j := 0; j <= varnumber; j++ {
			targets[i][j], _ = strconv.ParseFloat(strings.Trim(token2[j], " \r\n"), 64)
		}
	}
	defer fp.Close()
}
...
func compute_fitness(Prog []byte) float64 {
	...
	for i := 0; i < fitnesscases; i++ {
		...
		// added -- encourage simpler solutions
		if len >= 10 {
			fit *= (0.01 * float64(len))
		}
	}
	return (-fit)
}
