...
func fitness_function(Prog []byte) float64 {
    var result float64
    var fit float64 = 0.0
    _ = traverse(Prog, 0)
    for i := 0; i < fitnesscases; i++ {
...
func create_random_indiv(pop []byte, depth int) []byte {
    var population [MAX_LEN]byte
    var indlen = grow(population[:], 0, MAX_LEN, depth)
    for indlen < 0 {
        indlen = grow(population[:], 0, MAX_LEN, depth)
    }
...
func create_random_pop(pop [][]byte, n int, depth int, fitness []float64) [][]byte {
    for i := 0; i < n; i++ {
        //fmt.Printf( "%d ", i )
        //fmt.Println(" ")

        pop[i] = create_random_indiv(pop[i], depth)
        //fmt.Printf( "Call fitness_function() - %d\n", i )
        // fmt.Println(" ")
...
func simple_gp(fname string, s int64) {
    ...
    pop = create_random_pop(pop, POPSIZE, DEPTH, fitness[:])
    evolve(pop)
}
